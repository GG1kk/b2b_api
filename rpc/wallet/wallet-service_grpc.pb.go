...
func (c *walletServiceClient) CreateWalletForNewCompany(ctx context.Context, in *NewCompanyRequest, opts ...grpc.CallOption) (*CreateWalletForNewCompanyResponse, error) {
	out := new(CreateWalletForNewCompanyResponse)
	err := c.cc.Invoke(ctx, "/WalletService/CreateWalletForNewCompany", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _WalletService_CreateWalletForNewCompany_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(NewCompanyRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(WalletServiceServer).CreateWalletForNewCompany(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/WalletService/CreateWalletForNewCompany",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(WalletServiceServer).CreateWalletForNewCompany(ctx, req.(*NewCompanyRequest))
	}
	return interceptor(ctx, in, info, handler)
}
